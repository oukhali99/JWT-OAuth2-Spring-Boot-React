...
export const loginSuccessAction = createAction<{ token: string; username: string; id: number }>("auth/loginSuccessAction");
...
export const authenticate = (username: string, password: string) => async (dispatch: AppDispatch) => {
    const response = await apiActions.postRequest("/auth/authenticate", {}, {
...
export const authenticateOrRegisterWithGoogle = (accessToken: string) => async (dispatch: AppDispatch) => {
    const response = await apiActions.postRequest("/auth/authenticate-or-register-with-google", {}, {
        params: { accessToken },
    }) as AxiosResponse<ApiAuthResponse>;
...
export const register = (username: string, password: string) => async (dispatch: AppDispatch) => {
    const response = await apiActions.postRequest("/auth/register", {}, {
        params: { username, password },
    }) as AxiosResponse<ApiAuthResponse>;
